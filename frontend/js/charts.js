...
{label:'Equity',data:aa.map(d=>(d.equity||d.Equity||0)*100),backgroundColor:'rgba(59,130,246,.4)',borderColor:'rgba(59,130,246,.8)',fill:true,tension:.3,pointRadius:0},
{label:'Fixed Income',data:aa.map(d=>(d.bonds||d['Fixed Income']||0)*100),backgroundColor:'rgba(139,92,246,.4)',borderColor:'rgba(139,92,246,.8)',fill:true,tension:.3,pointRadius:0},
{label:'Alternative',data:aa.map(d=>(d.alternatives||d.Alternative||0)*100),backgroundColor:'rgba(16,185,129,.4)',borderColor:'rgba(16,185,129,.8)',fill:true,tension:.3,pointRadius:0},
{label:'Cash',data:aa.map(d=>(d.cash||d.Cash||0)*100),backgroundColor:'rgba(245,158,11,.4)',borderColor:'rgba(245,158,11,.8)',fill:true,tension:.3,pointRadius:0}
...
CH.heq=new Chart($('heq'),{type:'line',data:{labels,datasets:keys.map((k,i)=>({label:k,data:eq.map(d=>(d[k]||0)*100),backgroundColor:colors[i%7],borderColor:colors[i%7].replace('.5','.8'),fill:true,tension:.3,pointRadius:0}))},plugins:[hoverLine],options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{color:'#4b5563',font:{size:10}}},tooltip:ttOpts},scales:{x:{ticks:{color:'#6b7280',maxTicksLimit:10,font:{size:9}},grid:{display:false}},y:{stacked:true,min:0,ticks:{color:'#6b7280'},grid:{color:'#e5e7eb'}}}}})
}
if(h.fixed_income){
